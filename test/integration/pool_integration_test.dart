  ...
  group(
    ...
    () {
      ...
      test('poolClose releases all connections', () async {
        final dsn = test_env.getTestEnv('ODBC_TEST_DSN') ??
            test_env.getTestEnv('ODBC_DSN')!;
        final tempPoolId = native.poolCreate(dsn, 2);
        if (tempPoolId == 0) {
          fail('Failed to create pool');
        }

        // Get connections
        final connId1 = native.poolGetConnection(tempPoolId);
        final connId2 = native.poolGetConnection(tempPoolId);
        if (connId1 == 0 || connId2 == 0) {
          fail('Failed to get connections');
        }

        // Close pool
        final closeResult = native.poolClose(tempPoolId);
        expect(closeResult, isTrue);

        // Verify pool is closed
        final state = native.poolGetState(tempPoolId);
        expect(state, isNull);
      });
    },
