...
struct GlobalState {
    ...
    pools: HashMap<u32, ConnectionPool>,
    pooled_connections: HashMap<u32, PooledConnectionWrapper>,
    next_stream_id: u32,
    next_pool_id: u32,
...
pub extern "C" fn odbc_execute(
    ...
    let (conn_id, sql_str, timeout_sec) = match state.statements.get(&stmt_id) {
...
pub extern "C" fn odbc_pool_get_connection(pool_id: c_uint) -> c_uint {
    ...
    match pool.get() {
        Ok(pooled_wrapper) => {
            let conn_id = state.next_pooled_conn_id;
            state.next_pooled_conn_id += 1;
            state.pooled_connections.insert(conn_id, pooled_wrapper);
            conn_id
        }
        Err(e) => {
            set_error(
...
pub extern "C" fn odbc_pool_release_connection(connection_id: c_uint) -> c_int {
    ...
    if state.pooled_connections.remove(&connection_id).is_some() {
        0
    } else {
        set_error(
            &mut state,
...
pub extern "C" fn odbc_pool_get_state(
    ...
    // Caller ensures pointers are valid (null checks above).
    unsafe {
        *out_size = pool_state.size;
        *out_idle = pool_state.idle;
    }
...
/// Parallel bulk insert using pool. Stub: use Rust ParallelBulkInsert API for now.
#[no_mangle]
pub extern "C" fn odbc_bulk_insert_parallel(
    ...
    _columns: *const *const c_char,
    _column_count: c_uint,
    _data_buffer: *const u8,
    _buffer_len: c_uint,
    _parallelism: c_uint,
    _rows_inserted: *mut c_uint,
) -> c_int {
    let Some(mut state) = try_lock_global_state() else {
        return -1;
    };
    set_error(
        &mut state,
        "odbc_bulk_insert_parallel: use engine ParallelBulkInsert API".to_string(),
    );
    -1
}
