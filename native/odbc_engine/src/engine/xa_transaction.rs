...
impl XaTransaction {
    ...
    fn from_test_state(state: XaState, engine_id: &str, xid: Xid) -> Self {
        Self {
...
#[cfg(test)]
impl PreparingXa {
    fn from_test_transaction(inner: XaTransaction) -> Self {
        Self { inner }
    }
}
...
mod tests {
    use super::*;
    use crate::engine::core::{
        ENGINE_DB2, ENGINE_MARIADB, ENGINE_MYSQL, ENGINE_SQLITE, ENGINE_UNKNOWN,
    };
    ...
    fn prepared_xa_rollback_rejects_wrong_state() {
        ...
        let xa = PreparedXa {
            ...
            conn_id: u32::MAX,
            xid: sample_xid(),
            engine_id: ENGINE_MYSQL.to_string(),
            state: Arc::new(Mutex::new(XaState::Idle)),
            #[cfg(all(target_os = "windows", feature = "xa-dtc"))]
            dtc_branch: None,
        };
        let r = xa.rollback();
    ...
    fn parse_ascii_int_trims_and_parses() {
        assert_eq!(super::parse_ascii_int::<i32>(b" 42 "), Some(42));
        assert_eq!(super::parse_ascii_int::<usize>(b"0"), Some(0));
        assert_eq!(super::parse_ascii_int::<i32>(b"not-a-number"), None);
        assert_eq!(super::parse_ascii_int::<i32>(b"\xff"), None);
    }
    ...
    fn parse_ascii_int_parses_negative_i32() {
        assert_eq!(super::parse_ascii_int::<i32>(b"-99"), Some(-99));
    }
    ...
    fn hex_nibble_accepts_all_valid_ascii_hex_digits() {
        for (c, expected) in [
            ...
            assert_eq!(super::hex_nibble(c), Some(expected), "digit {c}");
        }
        assert_eq!(super::hex_nibble(b'g'), None);
    }
