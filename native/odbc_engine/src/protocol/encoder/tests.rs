use super::{
    checked_payload_add, checked_u16_len, checked_u32_len, EncodeError, RowBufferEncoder, MAGIC,
    OUTPUT_FOOTER_MAGIC, REF_CURSOR_FOOTER_MAGIC, VERSION,
};
...
fn try_append_output_footer_inserts_out1_and_params() {
    ...
    let out = RowBufferEncoder::try_append_output_footer(base.clone(), &[ParamValue::Integer(7)])
        .unwrap();
...
fn try_append_ref_cursor_footer_inserts_rc1_frame() {
    ...
    let out =
        RowBufferEncoder::try_append_ref_cursor_footer(base.clone(), std::slice::from_ref(&blob))
            .unwrap();
    let off = base.len();
...
fn test_encode_single_row_single_column() {
    ...
    let name_len =
        u16::from_le_bytes([encoded[metadata_offset + 2], encoded[metadata_offset + 3]]) as usize;
...
fn append_output_footer_result_maps_overflow_to_resource_limit_reached() {
    ...
    let base = RowBufferEncoder::encode(&RowBuffer::new()).unwrap();
    let out =
        RowBufferEncoder::append_output_footer_result(base.clone(), &[ParamValue::Null]).unwrap();
    assert!(out.len() > base.len());
...
#[test]
fn encode_with_compression_returns_empty_on_overflow() {
    let mut buffer = RowBuffer::new();
    for _ in 0..=u16::MAX {
        buffer.add_column(String::new(), OdbcType::Integer);
    }
