...
///
/// Implementation is split across capability delegates
/// ([TelemetryOdbcQueryDecorator], [TelemetryOdbcPoolDecorator],
/// [TelemetryOdbcAdminDecorator], [TelemetryOdbcTransactionDecorator]);
/// this class is a thin façade that forwards each call.
///
/// ## Usage
/// ```dart
...
class TelemetryOdbcServiceDecorator implements IOdbcService {
  ...
  TelemetryOdbcServiceDecorator(
    ...
    _admin = TelemetryOdbcAdminDecorator(service, ops);
    _query = TelemetryOdbcQueryDecorator(service, ops);
    _pool = TelemetryOdbcPoolDecorator(service, ops);
    _transaction = TelemetryOdbcTransactionDecorator(service, ops);
  }

  final IOdbcService _service;
  late final TelemetryOdbcAdminDecorator _admin;
  late final TelemetryOdbcQueryDecorator _query;
  late final TelemetryOdbcPoolDecorator _pool;
  late final TelemetryOdbcTransactionDecorator _transaction;
  ...
  Future<Result<T>> runInTransaction<T extends Object>(
    ...
    Duration? lockTimeout,
      ...
      _transaction.runInTransaction<T>(
        connectionId,
        action,
        isolationLevel: isolationLevel,
        savepointDialect: savepointDialect,
  ...
  Future<Result<T>> runInXaTransaction<T extends Object>(
    ...
    bool onePhase = false,
      ...
      _transaction.runInXaTransaction<T>(
        connectionId,
        xid,
        action,
        onePhase: onePhase,
