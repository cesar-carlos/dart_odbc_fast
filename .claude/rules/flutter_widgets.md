...
- ✅ Enforce with lints: `unnecessary_lambdas` and `implicit_call_tearoffs`

```dart
// ❌ Avoid: helper method only forwarding to a widget constructor
Widget buildUserCard(User user) => UserCard(user: user);

// ✅ Good: direct widget usage where adaptation is needed
itemBuilder: (context, index) => UserCard(user: users[index]);

// ✅ Good: constructor tear-off when signature matches
final icons = iconDataList.map(Icon.new).toList();

// ✅ Good: callback tear-off when signature matches
onPressed: _refresh;
```
