...
> **Last updated for:** v3.9.0 + `[Unreleased]` — `doc/CAPABILITIES_v3.md` is the canonical reference for engine capabilities, plugin traits and the capability × engine matrix. For the FFI surface see [API_SURFACE.md](API_SURFACE.md); for type mapping see [notes/TYPE_MAPPING.md](notes/TYPE_MAPPING.md).
...
MySQL/MariaDB and DB2 express waits in _seconds_; sub-second
...
covers the new SQL\_\* type codes (`SQL_GUID`=−11, `SQL_TYPE_TIME`=92, …).
...
- **Directional API:** `ParamDirection`, `DirectedParam` — DRT1 / `OUT1` and
  ...
  (`DirectedResultItem` / `DirectedRowCountItem`). **Ref-cursor** _wire_ (tag
